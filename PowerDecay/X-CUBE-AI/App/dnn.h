...
#define AI_DNN_MODEL_NAME          "dnn"

#define AI_DNN_ACTIVATIONS_ALIGNMENT   (4)

#define AI_DNN_IN_NUM       (1)
#define AI_DNN_IN { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_S8, 1, 12, 6, 1, NULL), \
}
#define AI_DNN_IN_SIZE { \
  (1 * 12 * 6), \
}
#define AI_DNN_IN_1_SIZE  (1 * 12 * 6)
#define AI_DNN_IN_1_SIZE_BYTES  ((1 * 12 * 6) * 1)





#define AI_DNN_OUT_NUM      (1)
#define AI_DNN_OUT { \
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_S8, 1, 1, 1, 1, NULL), \
}
#define AI_DNN_OUT_SIZE { \
  (1 * 1 * 1), \
}
#define AI_DNN_OUT_1_SIZE  (1 * 1 * 1)
#define AI_DNN_OUT_1_SIZE_BYTES  ((1 * 1 * 1) * 1)

#define AI_DNN_N_NODES (3)
 ...
 * @ref ai_dnn_create. Both the activations memory buffer 
 ...
 * header generated by the code generation tool. See AI_DNN_*
 * defines into file @ref dnn.h for all network sizes defines
 *
 * @param network an opaque handle to the network context
